...
5. 1이면 연결, 0이면 연결 X6. 마지막에는 여행 계획이 주어짐  
...
[개선지점]  
- union-find를 쓰면 된다 그것도 '겁나쉽게' ^^ ㅂㄷㅂㄷ  
- union-find : 그래프 알고리즘, 두 노드가 같은 그래프에 속하는지 판별해줌  
- union 연산(그래프 합치기) + find 연산(그래프에서 찾기)  
- 무식하게 걍 경유지 탐색으로 찾아봐야하면 분명 내가 한것처럼 재귀 for문 써도 됨 (or BFS)- 근데 비효율적임 ㅇㅇ, 알고리즘 귀찮다고 대충하고 도망치면 나중에 이렇게 또 마주함..  
...
if __name__ == '__main__':  
    ...
    print('YES' if all(parent[tour_plan[0]] == parent[_ - 1] for _ in tour_plan) else 'NO')